...
import type { InputModule, Module } from "../../src/core/Module";
...
import type { RawIOModule } from "../../src/types";
...
test("InputModule -> OutputPort -> Network", () => {
    ...
    //check if the module was generated correctly
    const moduleObj = result.get(0) as InputModule;
    expect(moduleObj).not.toBeUndefined();
    expect(moduleObj?.type).toBe(ObjectTypes.MODULE);
    expect(moduleObj?.id).toBe(0);
    expect(moduleObj?.inputs.length).toBe(0);
    expect(moduleObj?.outputs.length).toBe(1);
    expect(moduleObj?.moduleType).toBe(ModuleType.INPUT);
    expect(moduleObj?.state).toEqual(WireValue.unknown(16));
...
test("Network <-> InputPort <-> OutputModule", () => {
    const network = createRawNetwork(0, 16, [1]);
    const input = createRawInputPort(1, 16, 2);
    const module: RawIOModule = {
        id: 2,
    ...
    expect(moduleObj?.inputs.length).toBe(1);
    expect(moduleObj?.outputs.length).toBe(0);
    expect(moduleObj?.moduleType).toBe(ModuleType.OUTPUT);
    expect(moduleObj?.state).toEqual(WireValue.unknown(16));
